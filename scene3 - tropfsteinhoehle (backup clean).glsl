...
float distGlowingSpheres(vec3 point)
{
	// point = opRepeat(point, path(vec3(1, 1, 1)));
	// spherePos.xy += path(spherePos.z);
	vec3 spherePos = camP;
	// spherePos.xy -= 0.5*path(spherePos.z).xy;
	float sphereDist = sSphere(point,spherePos,0.05);
	return sphereDist;
}
...
float distField(vec3 pointHit)
{
	float totalDist = 0.0;
	totalDist = cave(pointHit);
	if(!traceWater)
		return totalDist;
	totalDist = min(totalDist,distWater(pointHit));
	if(!traceBalls)
		return totalDist;
	totalDist = min(totalDist,distGlowingSpheres(pointHit));
	return totalDist;
}
...
vec3 localShade(int id, vec3 point)
{
	...
	switch(id)
	{
		...
		case idGlowBall:
		{
			return ambientDiffuse(vec3(iGlobalTime),normal);
		}
		default:
			return -1;
	}
...
vec3 shade(int id, vec3 rayOrigin, vec3 camDir)
{
	vec3 color = localShade(id, rayOrigin);
	if(idWater == id)
	{
		vec3 normal = getNormal(rayOrigin, 0.01);
		vec3 rayDirection = reflect(camDir, normal);
		int blah = 0;
		float t = raymarchTerrain(rayOrigin, rayDirection, 0, 100, 100, blah);
		if(0 < t)
		{
			vec3 rayOrigin = rayOrigin + t * camDir;
			vec3 reflection = localShade(calculateID(rayOrigin), rayOrigin);
...
void main()
{
    ...
    vec2 aspectRatio = fragCoord.xy / iResolution.xy;
	vec2 uv = -1.0 + 2.0*aspectRatio;
	...
	camP = vec3(vec2(camPos.xy),path(camPos.z +2).z);
	vec3 light_pos = camP;
    ...
    // terrain	
	int iterations;
	float t = raymarchTerrain(camPos,rd,0,maxT,maxSteps,iterations);
    if( t>0.0 )
	{
		...
		float bac =  abs(dot( nor, rd));
        
        float ao = calculateAO(pos, nor);
		
		// vec3 lightDir = light_pos-pos;
		// float distlpsp = max(length(lightDir), 0.0001);
		// lightDir /= distlpsp;
		
	    // float diff = max( dot(nor, lightDir), 0.0);
		// float spec = pow(max( dot( reflect(-lightDir, nor), -rd ), 0.0 ), 8.);
...
/*void main( )
{
    ...
    // terrain	
	int iterations;
	float t = raymarchTerrain(rd,0,maxT,maxSteps,iterations);
    if( t>0.0 )
	{
		vec3 pos = ro + t*rd;
		vec3 nor = calcNormal( pos, t );
